...
export class Document {
    ...
    public executor: Executor = undefined!;
    ...
    // All the stack comments
    public rawComments: string[] = [];
    ...
    public executeExternalMacro(t: IToken): string[] {
        ...
        if (docToRun === undefined) {
            return this.executor.stack.stack;
        }

        const tempDoc = new Document(docToRun.rawContent);
        tempDoc.name = "VirtualExecutor - " + docToRun.name;
        const tempExecutor = new VirtualExecutor(
            docToRun.rawContent, 
            lexer.tokenize(docToRun.rawContent).tokens, 
            tempDoc,
            this,
            this.executor.stack
        );
        return tempExecutor.runMacro(macroDef, 1).stack;
    }
